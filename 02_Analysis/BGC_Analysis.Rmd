...
sample <- sort(list.files(path, pattern=".bgc.tsv", full.names = TRUE))

df.list <- lapply(sample, function(i) read.delim(i, sep = "\t", header = TRUE)[, c(13,28)])

# Assigning sample names to the ARG coverage data
names(df.list) <- sub("\\.bgc\\.tsv", "", basename(sample))

# loop through each dataset and add a column with 1
for (i in seq_along(df.list)) {
  df.list[[i]]$Count <- 1
}

# Ensure data structure is similar across all dataframes
# Filter the list to only keep dataframes with even columns
df.list <- Filter(function(x) ncol(x) == 3, df.list)
...
#
BGC_table <- purrr::reduce(df.list, dplyr::full_join, by = "pfam_ids")
colnames(BGC_table) <- c("BGC", samples)

BGC_table <- BGC_table %>% 
  as_tibble() %>%
  column_to_rownames(var = "BGC")

BGC_table[is.na(BGC_table)] <- 0
BGC_table <- ifelse(BGC_table > 0, 1, 0)
md <- md %>%
...
```{r Summary}
# Classified Bacteria
BGC_Summary <- data.frame(BGCs = rownames(BGC_table),
                          Prevalence = round(rowSums(BGC_table)/ncol(BGC_table),2)) %>%
  arrange(-Prevalence)

sum(BGC_Summary$Prevalence > 0.75)
```

```{r check collectors curve, echo=FALSE, warning=FALSE}
# Set a seed for reproducibility
set.seed(1234)
Bac_table.norm <- sweep(t(BGC_table),2,md$Bytes, "/") * 10^11
Bac_table.norm <- Bac_table.norm %>%
  t() %>%
  as_tibble()
Bac_table.norm <- round(Bac_table.norm,0)
rownames(Bac_table.norm) <- rownames(BGC_table)
...
Collectors_curve <- ggplot() + 
    ...
    geom_line(data = rare_list[[5]], 
              aes(x = observation, y = r), linewidth = 1.0, color = "grey20", alpha = 0.95, inherit.aes = FALSE) +
    xlab("Observations") +
    ylab("Observed No. BGC") +
    theme_light() + 
    ggtitle("Collectors curves")
      
```
```{r - plot BGCs}
#pdf("Collector_curve_BGC.pdf", height = 4, width = 6)
Collectors_curve
#dev.off()
```
...
BGC_counts <- data.frame("Run" = samples, "q0"=hill_div(BGC_table, qvalue = 0))
Lat_Long <- md %>%
...
res.ca <- ade4::dudi.coa(dist(t(BGC_table), method = "binary"), scannf = FALSE, nf = 2)
